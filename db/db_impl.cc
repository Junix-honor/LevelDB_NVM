...
Status DBImpl::NewDB() {
  ...
  if (use_nvm_mem_module) {
    new_db.SetMapNumber(2);
    //接下来新的文件会使用编号3
    new_db.SetNextFile(3);
  } else {
    //接下来新的文件会使用编号2
...
void DBImpl::CompactMemTable() {
  ...
  if (s.ok()) {
    ...
    if (use_nvm_mem_module) edit.SetMapNumber(mapfile_number_);
...
Status DB::Open(const Options& options, const std::string& dbname, DB** dbptr) {
  *dbptr = nullptr;
  ...
  if (s.ok() && save_manifest) {
    ...
    // TODO:MAX
    if (options.nvm_option.use_nvm_mem_module)
